package com.su.springsecurityjson.service;


import com.su.springsecurityjson.config.model.UrlResponse;
import com.su.springsecurityjson.dao.AuthorityUserRepository;
import com.su.springsecurityjson.util.SecurityResponse;
import com.su.springsecurityjson.util.TimeUtil;
...
public class UserService {
    ...
    public void findAllUserInfo1(String pageNum, String pageSize, SecurityResponse securityResponse) {
        int rowNum = Integer.parseInt(pageNum);
        int size = Integer.parseInt(pageSize);
        int row = (rowNum - 1) * size;
        List<Map<String, Object>> list = new ArrayList<>();
        List<Map<String, Object>> allUserInfo = authorityUserRepository.findAllUserInfo1(row, size);
        for (Map<String, Object> userInfo : allUserInfo) {
            int userId = (int) userInfo.get("id");
            List<Map<String, Object>> roleInfo = authorityUserRepository.findRoleInfoByUserId(userId);
            Map<String, Object> map = new HashMap<>();
        ...
        int userInfoSize = authorityUserRepository.findAllUserInfoSize1();
        securityResponse.setSuccess(true);
        securityResponse.setCode("1");
        securityResponse.setMessage("Find all user success!!");
        securityResponse.setData(list);
