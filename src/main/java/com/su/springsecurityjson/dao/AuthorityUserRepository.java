package com.su.springsecurityjson.dao;
...
public interface AuthorityUserRepository extends JpaRepository<AuthorityUser, Integer> {
    ...
    @Query(nativeQuery = true, value = "SELECT id AS id, username AS username, password AS password, nickname AS nickname, email AS email, phone AS phone, valid_time AS validTime, remark AS remark FROM authority_user LIMIT ?1,?2")
    List<Map<String, Object>> findAllUserInfo1(int pageNum, int pageSize);
    ...
    @Query(nativeQuery = true, value = "SELECT COUNT(id) FROM authority_user")
    int findAllUserInfoSize1();
