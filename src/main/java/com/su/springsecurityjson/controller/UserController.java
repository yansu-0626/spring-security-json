...
public class UserController {
     ...
     * 获取所有用户信息 (分页加模糊查询)
     */
    @PostMapping(value = "/user-manage/findAll1")
    public SecurityResponse findAll1(String pageNum, String pageSize) {
        try {
            if (!ObjectUtils.isEmpty(pageNum) && !ObjectUtils.isEmpty(pageSize)) {
                SecurityResponse securityResponse = new SecurityResponse();
                userService.findAllUserInfo1(pageNum, pageSize, securityResponse);
                return securityResponse;
            } else {
                return new SecurityResponse(false, "-1", "Incomplete information!!", "Incomplete information!!");
            }
